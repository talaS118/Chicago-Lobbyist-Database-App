...
def select_one_row(dbConn, sql, parameters = None):
    ...
    dbCursor = dbConn.cursor() ## start cursor

    ## try to execute sql query and return the data revtrieved
    try:
...
def perform_action(dbConn, sql, parameters = None):
    ...
    ## try to execute and commit sql action query and return the number of rows changed
    try:
        dbCursor.execute(sql, parameters)
        dbConn.commit()
